...
class PlcService {
  ...
  private async sendAndAwaitMatchingResponse (
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        ...
        reject(
          new Error(
            `Timeout waiting for response to N=${expectedTransition} for command '${commandString}'`
          )
        )
      }, timeoutMs)
      ...
      const onData = (data: Buffer) => {
        ...
        if (response.length < 25) {
          logger.warn(
            TAG,
            `Received a malformed or short response: ${response}`
          )
          return
        }
        ...
        if (responseTransition === expectedTransition) {
          ...
          logger.debug(
            TAG,
            `Ignored mismatched response (got N=${responseTransition}, want N=${expectedTransition})`
          )
        }
      }
    ...
    try {
      ...
      return await new Promise<DispenseResult>((resolve, reject) => {
        ...
        const onData = (data: Buffer) => {
          ...
          logger.debug(
            TAG,
            `Dispense listener (N=${transition}) received code T${responseCode}`
          )
  ...
  public async checkStatus (
    ...
    const response = await this.sendAndAwaitMatchingResponse(
      socket,
      commandString,
      transition
    )
  ...
  public async isTrayEmpty (
    ...
    try {
      ...
      logger.error(
        TAG,
        `Failed to check tray status for slot '${slot}':`,
        error
      )
      return false
    }
  ...
  public async turnOffLight (
    ...
    try {
      await this.sendAndAwaitMatchingResponse(
        socket,
        commandString,
        transition,
        1000
      ...
      logger.warn(
        TAG,
        `Could not confirm 'turnOffLight' (N=${transition}) was received, but continuing anyway.`
      )
    }
  }
}
